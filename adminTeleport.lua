...
--    ac.log("start")
--    --physics.setCarPenalty(ac.PenaltyType.ReleaseBlackFlag) --5)   # remove ac dsq
--    --physics.setCarPenalty(ac.PenaltyType.BlackFlag)           # dsq
--    --physics.setCarPenalty(ac.PenaltyType.TeleportToPits, 5)        
--    --physics.setCarPenalty(ac.PenaltyType.SlowDown, -5)   # add seconds penalty (ac slow down) 
...
local function teleportHUDClosed(okClicked)
  ...
  if okClicked and selectedCar then
    ...
    --ac.log(worldDirection)
    --ac.log(worldDirection[2] * -1)
